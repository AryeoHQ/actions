...
class ActionMustImplementShouldQueue extends AbstractRector
{
    ...
    public function refactor(Node $node): null|Node
    {
        ...
        return $this->addShouldQueue($node);
    }
    ...
    private function addShouldQueue(Class_ $node): Class_
    {
        // Add use statement for ShouldQueue interface
        try {
            $this->useNodesToAddCollector->addUseImport(
                new FullyQualifiedObjectType(ShouldQueue::class)
            );
        } catch (Throwable $e) {
            // In test environments, UseNodesToAddCollector might not have a current file
        }

        $shouldQueueInterface = new Name('ShouldQueue');

        if ($node->implements === []) {
            $node->implements = [$shouldQueueInterface];
        } else {
            $node->implements[] = $shouldQueueInterface;
        }
