...
`shouldExecute()` returns an instance of the Mockery `ExpectationInterface`, which allows you to chain any additional Mockery assertions.
...
A custom PHPStan rule is available to add to your projects to ensure Actions follow the implementation standards.

The rule can be added to your projects `phpstan.neon` to the `rules` key.

```yml
rules:
    - Tooling\Actions\PHPStan\Rules\ActionRule
```
